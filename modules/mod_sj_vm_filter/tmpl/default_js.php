...
$limit_results = trim((int)$params->get('limit_results'));
if($limit_results == 0 || $limit_results < 0){
	$limit_page = '5';
}else{
	$limit_page = trim($params->get('limit_results'));
}
...
jQuery(document).ready(function ($) {
	...
	(function (element) {
		...
		$_ajax_url = '<?php echo (string)JURI::current(); ?>';

		var $_gr_prices = $('.ft-group-prices', $element),
			$_slide_prices = $(".ft-slider-price", $_gr_prices),
			_min_price = <?php echo (int)$params->get('price_min',0); ?>,
			_max_price = <?php echo $price_max; ?>;
		var $timer = 0;
		...
		function updateAfterLoadAjax() {
            ...
            var pro_load = "<?php echo $limit_page; ?>";
			...
			_ft_opt_close.on('click', function () {
				...
				if (_cls_ft.length > 0) {
					...
					if (_data_value == 'ft-price-min') {
						$('.ft-price-min').val('<?php echo $price_min; ?>')
					}
					if (_data_value == 'ft-price-max') {
						$('.ft-price-max').val('<?php echo $price_max; ?>')
					}
