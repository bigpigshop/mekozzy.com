...
abstract class VMListingTabsHelper extends VMListingTabsBaseHelper
{
    ...
    public static function _getCategoryInfor($catids, $params)
    {
        ...
        if ($cat_ordering_direction == 'ASC') {
            switch ($cat_order_by) {
                default:
                case 'title':
                    usort($list, function($a, $b) {return strnatcasecmp( $a->category_name, $b->category_name);});
                    break;
                case 'created':
                    usort($list, function($a, $b){ return $a->created_on < $b->created_on;});
                    break;
                case 'random':
                    shuffle($list);
                    break;
        ...
        private static function _getPublishCategories($catids)
    {
        ...
        {
	        $q = "SELECT virtuemart_category_id as id
