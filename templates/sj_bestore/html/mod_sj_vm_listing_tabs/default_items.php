	...
	<?php if (!empty($child_items))
	{
		...
		?>
			<div class="item-inner">
				...
				?>
						<div class="item-image">
							<?php if ($params -> get('item_percentage_discount_display', 1) == 1)
							{
								...
								echo '<span class="onsale">' . JText ::_('Sale') . '</span>';
							} ?>
				...
				?>
				<?php
					if ($module -> id != 171 && (int)$params -> get('item_prices_display', 1) && (!empty($item -> prices['salesPrice']) || !empty($item -> prices['salesPriceWithDiscount']))) { ?>
					...
					<?php
						...
						if (empty($item -> rating)) {
							...
							<div class="ratingbox dummy"
								 title="<?php echo vmText ::_('COM_VIRTUEMART_UNRATED'); ?>"></div>
							<?php
						} else {
							$ratingwidth = $item -> rating * 13;
							?>

							<div title=" <?php echo(vmText ::_("COM_VIRTUEMART_RATING_TITLE") . round($item -> rating) . '/' . $maxrating) ?>"
								 class="ratingbox">
			...
			<?php
				if ($params -> get('type_show') == 'slider' && ($i % $params -> get('nb_rows') == 0 || $i == $count))
			{ ?>
			</div>
		<?php }
			if ($params -> get('type_show') == 'loadmore')
