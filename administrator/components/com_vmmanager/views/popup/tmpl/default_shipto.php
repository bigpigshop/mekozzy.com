...
$data = file_get_contents('components/com_vmmanager/helpers/config/shipTo.json');
...
$userFields = $userModel->getUserFieldsFor('cart','ST');
$s = array();
$fieldST = $userModel->getUserFieldsFilled($userFields,$s,'');
$fields = $this->getFieldInfo($fieldST['fields']);
...
<div id="popupItem">
    ...
    <div id="privateItem" class="active">
        <form method="POST" action="#" id="optionForm" name="optionForm">
            <div class="baner-row">
                <div class="title-and-desc">
                    <h3 class="title"><?php echo JText::_('OPC_POPUP_OPTION_DISABLE_SAME_BILLTO') ?></h3>
                    <p class="desc"><?php echo JText::_('OPC_POPUP_OPTION_DISABLE_SAME_BILLTO_DESC') ?></p>
                </div>
                <div class="row-val">
                    <span class="onoffswitch">
                    <input <?php if($dataOption->disable_same_billto):?> checked="" <?php endif; ?>  type="checkbox" value="1" name="disable_same_billto" class="onoffswitch-checkbox" id="disable_same_billto"/>
                        <label class="onoffswitch-label" for="disable_same_billto">
            ...
            <div class="description-example-color">
                <p>
                    <?php echo JText::_('OPC_POPUP_SHIPTO_DESC') ?>
            ...
            <ul class="privateItem">
                <?php $i=0; foreach($fields as $field): ?>
                    ...
                    <li class="fieldItem Shipto" sort-name="<?php echo $field->name ?>" old_sort="<?php echo $i; $i++ ?>">
                    <?php if(!in_array($field->name,$coreField)): ?>
                    <i data="<?php echo $field->virtuemart_userfield_id?>" class="close-yesorno close-field opc-iconcancel-1" title="Unpublish"></i>
                    <?php endif; ?>
